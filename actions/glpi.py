...
def load_glpi_config(config_file="glpi_credentials.yml"):
    ...
    glpi_api_uri = (
        os.environ.get("GLPI_API_URI")
        if os.environ.get("GLPI_API_URI")
        else glpi_config.get("uri")
    )
    glpi_app_token = (
        os.environ.get("GLPI_APP_TOKEN")
        if os.environ.get("GLPI_APP_TOKEN")
        else glpi_config.get("app_token")
    )
    glpi_auth_token = (
        os.environ.get("GLPI_AUTH_TOKEN")
        if os.environ.get("GLPI_AUTH_TOKEN")
        else glpi_config.get("auth_token")
    )
    local_mode = (
        os.environ.get("GLPI_LOCALMODE")
        if os.environ.get("GLPI_LOCALMODE")
        else glpi_config.get("localmode", "True")
    )
    local_mode = (
        json.loads(local_mode.lower())
        if local_mode is not None and type(local_mode) == str
        else local_mode
    )
...
class GLPIService(object):
    ...
    def __init__(self, uri, app_token, auth_token, *args, **kwargs):
        if not GLPIService.__instance:
            ...
            if uri is None or app_token is None or auth_token is None:
                raise GlpiException(
                    f"parameters missing:  uri{uri is None}| app_token: {app_token is None}"
    ...
    def get_ticket_status(self, ticket_id: Text):
		...
		"""
        metacriteria = [
            {"id": "1", "field": "title"},  # title
            {"id": "7", "field": "category"},  # category name
            {"id": "2", "field": "id"},
            {"id": "14", "field": "type"},
            {"id": "12", "field": "status"},
            {"id": "4", "field": "user_id"},  # name
            {"id": "9", "field": "source"},  # name
            {"id": "35", "field": "use_notification"},
            {"id": "34", "field": "alternative_email"},  # name
            {"id": "10", "field": "urgency"},
            {"id": "11", "field": "impact"},
            {"id": "3", "field": "priority"},
            {"id": "15", "field": "date"},  # opendate
            {"id": "16", "field": "closedate"},
            {"id": "17", "field": "solvedate"},
            {"id": "18", "field": "time_to_solve"},
            {"id": "19", "field": "date_mod"},
        ]
        full_url = (
            f"{self.base_uri}/search/ticket?criteria[0][field]=2&criteria[0][value]={ticket_id}"
    ...
    def get_user(self, username: Text = "", user_id: int = None):
        ...
        user_criteria: Dict = {
            ...
            "metacriteria": [
                {"id": "1", "field": "name"},
                {"id": "2", "field": "id"},
                {"id": "9", "field": "firstname"},
                {"id": "34", "field": "realname"},
                {"id": "5", "field": "email"},
                {"id": "8", "field": "is_active"},
            ],
        }
    ...
    def create_ticket(self, ticket: Ticket, ticket_type: int = 1) -> Dict[Text, Any]:
        ...
        new_ticket: Dict = {
            ...
            "requesttypes_id": requesttypes_id_default
            if "requesttypes_id" not in ticket
            else ticket["requesttypes_id"],
            "content": ticket["description"],
            # 'urgency': 3,  # medium
            # 'impact': 2,  # low
            # 'priority': ticket['priority'],  # low:2 | medium:3 | high:4
            "itilcategories_id": itilcategories_id_default
            if "itilcategories_id" not in ticket
            else ticket["itilcategories_id"],
            "type": ticket_type,
    ...
    def build_user_query(self, criteria: Dict, opts) -> Text:
        ...
        for field_id, field_opts in viewitems(opts):
            if field_id.isdigit() and "uid" in field_opts:
                ...
                field_name = re.sub(
                    "^" + item_name + ".", "", field_opts["uid"], flags=re.IGNORECASE
                )
    ...
    def assign_ticket(self, ticket: Ticket, user_id: int = None):
        ...
        if user_id is not None or "alternative_email" in ticket:
            ...
            if "alternative_email" in ticket:
                requester_payload["input"][0]["alternative_email"] = ticket[
                    "alternative_email"
                ]
